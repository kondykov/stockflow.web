...
import type { PermissionOption } from "~/types/auth";
...
async function handleSave() {
  saving.value = true;
  try {
    const response = await useApi(`/api/identity/rbac/role`, {
      method: 'POST',
      body: {
        name: roleName.value,
    ...
    if (response.successful) {
      toast.add({ title: 'Успешно', description: 'Роль создана', color: 'success' });
...
<template>
  <UContainer class="py-6 space-y-6">
    ...
    <div class="flex items-center justify-between gap-4 bg-white dark:bg-gray-900 sticky top-0 z-20 py-4 border-b border-gray-200 dark:border-gray-800">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
      ...
      <div class="lg:col-span-3">
        ...
        <div v-else class="space-y-12">
          ...
          >
            <!-- Group Header -->
            <div class="flex items-center gap-4 sticky top-[88px] bg-white dark:bg-gray-900 z-10 py-2">
